...
setwd("/Dokumente und Einstellungen/admin/OneDrive/PhD/Data_mining/fastAAI/fastAAI_EX_MAGs_v3/results/")
load("../fastAAI_heatmap.RData")

# Load data ####
all.files <- list.files(pattern = ".*.txt")
data.list <- lapply(all.files, function(x)read.table(x, sep="\t", header=T))
data.list <- lapply(data.list, function(x) x[,c(1,2,7)])
df <- bind_rows(data.list)
cols = c("query", "target")
df <- df[which(!duplicated(t(apply(df[cols], 1, sort)))), ]
 
# create table for supplemental 
write.table(df, file = "../AAI_table_combined.txt", col.names =T, row.names = F, sep="\t")

# load table
fastAAI_table <- read.table("../AAI_table_combined.txt", header=T,sep="\t")

# Create matrix ####
# extract unique target genomes
target_genomes <- unique(fastAAI_table$target)

# extract unique query genomes
query_genomes <- unique(fastAAI_table$query)

## Create an empty matrix with dimensions based on the unique genomes ####
AAI_matrix <- matrix(nrow = length(target_genomes), ncol = length(target_genomes),
                     dimnames = list(target_genomes, target_genomes))

## Fill in the matrix with average nucleotide identity values ####
for (i in 1:nrow(fastAAI_table)) {
  target_genome <- fastAAI_table[i, "target"]
  query_genome <- fastAAI_table[i, "query"]
  aai_value <- fastAAI_table[i, "AAI_estimate"]
  AAI_matrix[target_genome, query_genome] <- aai_value
}


## Convert matrix to a data frame for tabular representation ####
AAI_matrix_df <- melt(AAI_matrix)
colnames(AAI_matrix_df) <- c("target","query","AAI")
...
reversed_table <- AAI_matrix_df
reversed_table <- reversed_table[c("target", "query", "AAI")]
colnames(reversed_table) <- c("query", "target", "AAI")
merged_table <- merge(AAI_matrix_df, reversed_table, by = c("query", "target"), all = TRUE)

# Combine the separate AAI columns into a single AAI column
merged_table$AAI <- ifelse(is.na(merged_table$AAI.x), merged_table$AAI.y, merged_table$AAI.x)

# Remove the separate AAI columns if needed
merged_table <- subset(merged_table, select = -c(AAI.x, AAI.y))

merged_table$AAI[merged_table$AAI == '>90%'] <- 90

merged_table$AAI <- as.numeric(merged_table$AAI)
merged_table$query <- gsub(".fasta","",merged_table$query)
merged_table$query <- gsub(".fa","",merged_table$query)
merged_table$target <- gsub(".fasta","",merged_table$target)
merged_table$target <- gsub(".fa","",merged_table$target)

# reorder the levels
levels(merged_table$query)
levels(merged_table$target)
...
## plot heatmap ####
# This is mainly used to make sure that the heatmap looks okay based on its order
ggplot(merged_table_1, aes(x = query, y = target, fill = AAI)) +
  geom_tile(color = "#f0f0f0") +
  geom_text(aes(label = round(AAI)), size = 3) +
...
ggsave("../FastAAI_EX_MAGs_Heatmap.png", plot = last_plot(), width = 15, height = 15, units = "cm", device = "png" )


# Plot triangle heatmap ####

#transform merged_table into matrix
...
for (i in 1:nrow(merged_table_2)) {
  ...
  aai_value <- merged_table_2[i, "AAI"]
  merged_table_matrix[target_genome, query_genome] <- aai_value
}
...
colnames(upper_tri_df) <- c("target","query","AAI")



## plot heatmap ####
ggplot(upper_tri_df, aes(x = target, y = query, fill = AAI)) +
  geom_tile(color = "#f0f0f0") +
  geom_text(aes(label = round(AAI)), size = 2.5) +
  scale_fill_gradientn("AAI [%]",colours = c("#00441b","#5aae61","#d9f0d3","#e7d4e8","#9970ab","#40004b"),
                       na.value = "white", limits = c(40,90),
                       breaks = c(40, 50, 60, 70, 80, 90), labels = c("40","50", "60", "70", "80", ">90")) +
  coord_fixed() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.25, hjust=1, size = 6),
        axis.text.y = element_text(size = 6),
        axis.title.x = element_blank(),
...
ggsave("../FastAAI_EX_MAGs_Heatmap_upper_tri.png", plot = last_plot(), width = 18, height = 18, units = "cm", device = "png" )




# Plot AAI levels as colors in legend ####

## Konstantinidis et al 2022 - fastAAI ####
# Domain: 0 - 35.3%
# Phylum: - 40.5%
# Class: - 45.0%
# Order - 50%
...
# Genus: - 62.3%
test <- upper_tri_df
test$group <- factor("sreened")
test2 <- test %>% 
    mutate(group = factor(if_else(AAI >= 53 , "Genus", 
                                          if_else(AAI >= 50 & AAI < 53, "Family",
                                                  if_else(AAI >= 45 & AAI < 50, "Order",
                                                          if_else(AAI >= 40.5 & AAI < 45, "Class", "Phylum"))))))

test2$group <- factor(test2$group, levels=rev(c("Species","Genus", "Family", "Order", "Class", "Phylum")), ordered=T)

library(RColorBrewer)

ggplot(test2, aes(x = target, y = query, fill = group)) +
  geom_tile(color = "#f0f0f0") +
  coord_fixed() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.25, hjust=1, size = 6),
        axis.text.y = element_text(size = 6),
        ...
        legend.text = element_text(size = 7),
        legend.title = element_text(size = 7, face = "bold"))+
  scale_fill_brewer(breaks = levels(test2$group), palette = "YlGnBu", na.value = "white")
                    

ggsave("../FastAAI_EX_MAGs_Heatmap_upper_tri_group.png", plot = last_plot(), width = 15, height = 15, units = "cm", device = "png" )


## Konstantinidis et al 2018 ####
test3 <- test %>% 
  mutate(group = factor(if_else(AAI >= 65 , "Genus", 
                                if_else(AAI > 45 & AAI < 65, "Family", "Order"))))
                                        

test3$group <- factor(test3$group, levels=rev(c("Genus", "Family", "Order")), ordered=T)

library(RColorBrewer)

ggplot(test3, aes(x = target, y = query, fill = group)) +
  geom_tile(color = "#f0f0f0") +
  coord_fixed() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.25, hjust=1, size = 6),
        axis.text.y = element_text(size = 6),
...
ggsave("../FastAAI_EX_MAGs_Heatmap_upper_tri_group_Konstantinidis.png", plot = last_plot(), width = 15, height = 15, units = "cm", device = "png" )

save.image(file = "../fastAAI_heatmap.RData")
load("../fastAAI_heatmap.RData")
