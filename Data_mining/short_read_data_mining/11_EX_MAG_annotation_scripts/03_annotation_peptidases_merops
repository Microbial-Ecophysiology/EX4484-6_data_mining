...
# plot heatmap
ggplot(matrix_Pep_melt[], aes(x = genome, y = merops_subfamily)) +
  ...
  scale_size_area("peptidase counts",max_size = 6)
...
matrix_SP <- table(SPs_families$genome, SPs_families$merops_subfamily)
matrix_SP_melt <- melt(matrix_SP)
colnames(matrix_SP_melt) <- c("genome","merops_subfamily","value")

matrix_SP_melt$genome <- factor(matrix_SP_melt$genome, levels = c("SUTE22-1_SAMN10231914_MAG_00000186","SUTE22-1_SAMN10231894_MAG_00000173","SUTE22-1_SAMN10231913_MAG_00000219","SUTE22-1_SAMN10231893_MAG_00000067",
                                                                  "SUTE22-1_SAMN10231914_MAG_00000292","SUTE22-1_SAMN10231913_MAG_00000177",
                                                                  "SUTE22-1_SAMN10231904_MAG_00000012","SCRA20-1_SAMN11854494_MAG_00000079","PRJNA704804_bin_147_orig_refined-contigs","TARA_SAMEA2620113_MAG_00000097",
                                                                  "PRJNA541421_bin_98_orig_refined-contigs","PRJNA531756_bin_72_orig_refined-contigs",
                                                                  "GCA_021160765.1_ASM2116076v1_genomic","GCA_021158585.1_ASM2115858v1_genomic",
...
ggplot(matrix_SP_melt, aes(x = genome, y = merops_subfamily)) +
  geom_point(aes(size=value, fill=value, col=value), shape = 21)+
  theme_bw() +
  coord_flip() +
  scale_y_discrete(position="right")+
  ...
  theme(axis.text.x = element_text(angle=-90, hjust=1, vjust=-0.8, color='grey40', size=7),
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())+
  scale_color_viridis_c("signal peptidase counts",guide = "legend", option="mako", direction=-1,breaks = c(0, 1, 2))+
  scale_fill_viridis_c("signal peptidase counts",guide = "legend", option="mako", direction=-1, breaks = c(0, 1, 2))+
  scale_size_area("signal peptidase counts",max_size = 6,
                  breaks=c(0,1,2),
                  labels = c("0","1","2"))

ggsave("Peptidases/Peptidase_SPs_families_overview.png", plot = last_plot(), width = 12, height = 15, units = "cm", device = "png" )
ggsave("Peptidases/Peptidase_SPs_families_overview.pdf", plot = last_plot(), width = 12, height = 15, units = "cm", device = "pdf" )
